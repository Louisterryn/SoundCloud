<template>
  <div>
      ...
      <div class="row lignePrincipale">
        <div class="col-sm-2 navSoundcloud" id="sticky-sidebar">
          ...
          <ul class="list-group list-group-flush grpListSoundcloud">
            ...
            <li class="list-group-item lstSoundcloud" @click="affichageFunk()">
              Rock
            </li>
            <li class="list-group-item lstSoundcloud" @click="affichageRock()">
              Pop
            </li>
            <li class="list-group-item lstSoundcloud" @click="affichageBlues()">
              Punk
            </li>
          </ul>
        </div>
        <div class="col-sm-10 contentSoundcloud">
          ...
          <div class="composantMusique">
            <musique
              v-if="affiche < 1"
              @click.native="play(musiqueIndex)"
              v-for="(Musique, musiqueIndex) in Musiques"
              :son="Musique"
              :key="musiqueIndex"
            />
            <musique
              v-if="affiche == 1 && Musique.Genre == 'Funk'"
              @click.native="play(musiqueIndex)"
              v-for="(Musique, musiqueIndex) in Musiques"
              :son="Musique"
              :key="musiqueIndex"
            />
            <musique
              v-if="affiche == 2 && Musique.Genre == 'Rock'"
              @click.native="play(musiqueIndex)"
              v-for="(Musique, musiqueIndex) in Musiques"
              :son="Musique"
              :key="musiqueIndex"
            />
            <musique
              v-if="affiche == 3 && Musique.Genre == 'Blues'"
              @click.native="play(musiqueIndex)"
              v-for="(Musique, musiqueIndex) in Musiques"
              :son="Musique"
              :key="musiqueIndex"
    ...
    <div class="mainContainer">
      ...
      <div
        class="mainFavoris"
        v-for="(musique, index) in Musiques"
        :key="index"
      >
        Titre : {{ musique.Titre }} Son :{{ musique.Artiste }}
...
export default {
  ...
  methods: {
    ...
    play(position) {
      ...
      this.imgAritste.backgroundImage =
        "url(" + this.Musiques[position].Images + ")";
